import { Search, ClipboardList, Code, TestTube, Rocket, Wrench } from 'lucide-react';

export function SoftwareProcess() {
  const steps = [
    { icon: <Search className="w-7 h-7" />, title: 'Discovery', description: 'Understanding requirements and scope', number: '01' },
    { icon: <ClipboardList className="w-7 h-7" />, title: 'Planning', description: 'Architecture and roadmap creation', number: '02' },
    { icon: <Code className="w-7 h-7" />, title: 'Development', description: 'Building your software solution', number: '03' },
    { icon: <TestTube className="w-7 h-7" />, title: 'Testing', description: 'Quality assurance and bug fixing', number: '04' },
    { icon: <Rocket className="w-7 h-7" />, title: 'Deployment', description: 'Launch and go-live process', number: '05' },
    { icon: <Wrench className="w-7 h-7" />, title: 'Maintenance', description: 'Ongoing support and updates', number: '06' },
  ];

  return (
    <section className="software-process-section w-full py-32" style={{ backgroundColor: '#FFFFFF' }}>
      <div className="max-w-[1440px] mx-auto px-12">
        <div className="text-center mb-20 space-y-4">
          ...
          <h2 style={{ fontFamily: 'Inter, sans-serif', fontSize: '48px', fontWeight: 700, color: '#151561', letterSpacing: '-0.02em', lineHeight: '1.2' }}>How We Build Software</h2>
        </div>
